...
import { FISCAL_DISPATCHABLE_STATUSES } from '../constants/fiscal-dispatch.constants';
...
import { NfeDispatchContext } from '../types/nfe-dispatch.types';

@Injectable()
export class NfeDispatchValidator {
  constructor(
  ...
  async validate(payload: DispatchNfeDto): Promise<NfeDispatchContext> {
    const [transaction, fiscalData, emitter] = await Promise.all([
      this.loadTransaction(payload),
      this.loadFiscalData(payload),
      this.loadEmitter(payload.emitter),
    ]);

    this.assertFiscalDataBelongsToTransaction(fiscalData, transaction);
    this.assertTransactionPayloadMatches(transaction, payload);

    return { transaction, fiscalData, emitter };
  }
  ...
  private async loadTransaction(payload: DispatchNfeDto): Promise<Transaction> {
  ...
  private async loadFiscalData(payload: DispatchNfeDto): Promise<TransactionFiscalData> {
  ...
  private async loadEmitter(emitter: EmitterDto): Promise<Emitter> {
